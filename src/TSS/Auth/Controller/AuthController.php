...
namespace TSS\Auth\Controller;


use TSS\Auth\Entity\AbstractCredential;
use TSS\Auth\Entity\AbstractUser;
use TSS\Auth\Form\SigninForm;
use TSS\Auth\Form\SignupForm;
...
class AuthController extends AbstractActionController
{
    public function signinAction()
    {
        ...
        if ($this->identity()) {
            return $this->redirect()->toRoute($config['tss']['auth']['routes']['redirect']['name']);
        }

        $form = new SigninForm();
        $form->setAttribute('action', $this->url()->fromRoute($config['tss']['auth']['routes']['authenticate']['name']));
        $form->prepare();

        $viewModel = new ViewModel(array(
            'form' => $form,
            'authRoutes' => $config['tss']['auth']['routes']
        ));

        $viewModel->setTemplate($config['tss']['auth']['template']['signin']);

        $this->layout($config['tss']['auth']['layout']);
    ...
    public function authenticateAction()
    {
        ...
        return $this->redirect()->toRoute($config['tss']['auth']['routes']['signin']['name']);
    }
    ...
    public function signupAction()
    {
        ...
        $form->setAttribute('action', $this->url()->fromRoute($config['tss']['auth']['routes']['signup']['name']));
        $user = new $config['tss']['auth']['config']['identityClass'];
        $form->bind($user);
        $form->prepare();
        ...
        if ($request->isPost()) {
            ...
            if ($form->isValid()) {
                $credential = new $config['tss']['auth']['config']['credentialClass']();
                $credential->setType(AbstractCredential::TYPE_PASSWORD);
                $credential->setValue(sha1(sha1($form->get('password')->getValue())));
                $credential->setUser($user);
                $user->addCredential($credential);

                $role = $entityManager->find($config['tss']['auth']['config']['roleClass'], $config['tss']['auth']['config']['roleDefault']);
                $user->setRole($role);

                $user->setAvatar($this->imageThumb()->getDefaultImageThumb());
                $user->setActive($config['tss']['auth']['config']['identityActive']);
                ...
                $fullLink = $this->url()->fromRoute(
                    $config['tss']['auth']['routes']['confirm-email']['name'],
                    array(
                        'token' => $user->getToken(),
                    ),
        ...
        $viewModel->setTemplate($config['tss']['auth']['template']['signup']);
    ...
    public function confirmEmailAction()
    {
        ...
        $entityManager = $this->getServiceLocator()->get('Doctrine\ORM\EntityManager');
        $identityRepo = $entityManager->getRepository($config['tss']['auth']['config']['identityClass']);
